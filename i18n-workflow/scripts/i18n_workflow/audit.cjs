...
function getSpriteFrameMapSource(config) {
...
function parseSpriteFrameMapFromSource(config, targetLanguages) {
  const mapSource = getSpriteFrameMapSource(config);
  if (!mapSource) return {};
  const result = {};
...
function parseSpriteFrameMap(config, targetLanguages) {
...
function scanPrefabLikeFile(filePath, spriteFrameIndex, config) {
...
function resourcePathExists(resourcesPath, config) {
  if (!resourcesPath) return false;
  const absolute = path.join(config.resourcesRoot, ...resourcesPath.split('/'));
...
function buildI18nIssues(spriteFrameMap, spriteFrameIndex, usedSpriteUuids, config) {
  const mappedMissing = [];
  ...
  for (const [uuid, mapping] of Object.entries(spriteFrameMap)) {
    ...
    for (const [language, targetPath] of Object.entries(mapping)) {
      if (!targetPath || !resourcePathExists(targetPath, config)) {
        mappedMissing.push({
          spriteFrameUuid: uuid,
          language,
...
function runAudit(config) {
  ...
  const spriteFrameIndex = loadSpriteFrameIndex(config, resourcesRelPrefix);
  ...
  for (const filePath of scannedFiles) {
    const result = scanPrefabLikeFile(filePath, spriteFrameIndex, config);
    if (result.parseError) {
      parseErrors.push(rel(filePath, config));
      continue;
    }
  ...
  const i18n = buildI18nIssues(spriteFrameMap, spriteFrameIndex, usedSpriteUuids, config);
