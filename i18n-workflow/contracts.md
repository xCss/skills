...
module.exports = {
  ...
  // --- Standard i18n Runtime (optional but recommended for H5/B-scheme projects) ---
  runtime: {
    platform: 'h5',
...
## Runtime Audit Report Schema

Generated by `run --steps runtime --dry-run` as `i18n-runtime-audit.json`.
...
```jsonc
{
  ...
  "languageResolutionCases": [
    { "input": ["zh-CN"], "resolvedLanguage": "zh" },
    { "input": ["en-US"], "resolvedLanguage": "en" },
    { "input": ["fr-FR"], "resolvedLanguage": "en" }
  ],
  "warnings": [],
  "problems": []
}
...
For H5 projects, `runtime.initBeforeFirstScene` must be true. If language initialization happens after the first real UI scene, users can see source-language text before the target/browser language is applied.
