...
function setContext(nextConfig) {
  ...
  reportPath = path.join(config.reportDirectory, 'i18n-asset-audit.json');
  manifestPath = path.join(config.reportDirectory, 'i18n-image-manifest.json');
  resourcesRoot = config.resourcesRoot;
  targetLanguages = deriveTargetLanguages(config);
}
...
async function runImages(nextConfig, argv) {
  setContext(nextConfig);
  ...
  if (args.writeTemplate) {
    ...
    return { action: 'write-template', output };
  }
  ...
  if (args.applyTranslations) {
    ...
    saveManifest(manifest);
    return { action: 'apply-translations', applied, summary: manifest.summary };
  }

  if (args.classify) {
    await runClassify(args);
    return { action: 'classify', summary: loadManifestOrBuild().summary };
  }

  if (args.generate) {
    await runGenerate(args);
    return { action: 'generate', summary: loadManifestOrBuild().summary };
  }
